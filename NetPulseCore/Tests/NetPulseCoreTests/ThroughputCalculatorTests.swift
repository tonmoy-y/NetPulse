...
final class ThroughputCalculatorTests: XCTestCase {
    func testBasicDelta() throws {
        ...
        let sample = try XCTUnwrap(ThroughputCalculator.sample(previous: prev, current: cur))
    ...
    func testCounterResetClampsToZero() throws {
        let t0 = Date()
        let t1 = t0.addingTimeInterval(1)
    ...
    func testVeryHighThroughput() throws {
        let t0 = Date()
